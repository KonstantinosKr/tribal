...
void loba_migrateGhosts(struct loba *lb, int  myrank, unsigned long long int size, unsigned int *nt, iREAL *t[3][3], iREAL *v[3], iREAL *p[3], iREAL *q[3], iREAL *distance, unsigned int *tid, unsigned int *pid)
{
    ...
    for(int i=0;i<nNeighbors;i++)
    {
      ...
      if(rcvpivot[i] > 0)
      {//safe check
        MPI_Irecv(&rcvtid_buffer[i][0], rcvpivot[i], MPI_INT, proc, 0, MPI_COMM_WORLD, &myrvRequest[(i*6)]);
        MPI_Irecv(&trvbuffer[0][(i*size*3)], rcvpivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myrvRequest[(i*6)+1]);
        MPI_Irecv(&trvbuffer[1][(i*size*3)], rcvpivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myrvRequest[(i*6)+2]);
        MPI_Irecv(&trvbuffer[2][(i*size*3)], rcvpivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myrvRequest[(i*6)+3]);
        
        MPI_Irecv(&vrvbuffer[(i*size*3)], rcvpivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myrvRequest[(i*6)+4]);
        MPI_Irecv(&rcvpid_buffer[i][0], rcvpivot[i], MPI_INT, proc, 0, MPI_COMM_WORLD, &myrvRequest[(i*6)+5]);
      }
    }
    ...
    for(int i=0;i<nNeighbors;i++)
    {
      ...
      if(pivot[i] > 0)
      {//safe check
        MPI_Isend(&tid_buffer[i][0], pivot[i], MPI_INT, proc, 0, MPI_COMM_WORLD, &myRequest[(i*6)]);
        MPI_Isend(&tbuffer[0][(i*size*3)], pivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myRequest[(i*6)+1]);
        MPI_Isend(&tbuffer[1][(i*size*3)], pivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myRequest[(i*6)+2]);
        MPI_Isend(&tbuffer[2][(i*size*3)], pivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myRequest[(i*6)+3]);
        
        MPI_Isend(&vbuffer[(i*size*3)], pivot[i]*3, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD, &myRequest[(i*6)+4]);
        MPI_Isend(&pid_buffer[i][0], pivot[i], MPI_INT, proc, 0, MPI_COMM_WORLD, &myRequest[(i*6)+5]);
      }
    }
